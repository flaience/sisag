...
const metric = (overrides = {}) => ({ planId: "plan-1", candidateId: "candidate-1", executions: 50, successRate: 99, fallbackRate: 1, p95DurationMs: 800, averageTokens: 900, coverage: { modes: 50, durations: 50, tokens: 50 }, ...overrides });
const render = (current = [metric()], previous = [metric()]) => {
  const data: StableHealthData = {
    ...
    health: evaluateRecoveryRetrievalStableHealth({ current, previous, complete: true }), readOnly: true, automaticAction: false,
  };
...
describe("stable health evidence UI", () => {
  it.each([
    ...
    [{ successRate: 80, fallbackRate: 20 }, "Crítico"],
    [{ successRate: 80, fallbackRate: 1 }, "Dados insuficientes"],
  ...
  it("shows plan, candidate, sample and policy", () => {
    const html = render();
    for (const text of ["plan-1", "candidate-1", "50 execuções", "mínimo da política: 50", "recovery_retrieval_stable_health_v2"]) expect(html).toContain(text);
  });
  ...
  it("does not display unmeasured latency as passing", () => {
    const html = render([metric({ coverage: { modes: 50, durations: 0, tokens: 50 }, p95DurationMs: null })]);
    expect(html).toContain("Dados insuficientes");
    expect(html).toContain("Não avaliado");
    expect(html).not.toContain("Os indicadores avaliados atendem");
  });
