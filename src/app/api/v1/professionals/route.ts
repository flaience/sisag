...
export async function GET(request: NextRequest) {
  try { const auth = await requireApiRole(request, ["owner", "admin", "staff"]); if (auth.ok === false) return auth.response; const params = new URL(request.url).searchParams; const search = params.get("search") ?? ""; const requestedStatus = params.get("status"); const status = requestedStatus === "inactive" || requestedStatus === "all" ? requestedStatus : "active"; const unitId = params.get("unitId")?.trim() ?? ""; const items = unitId ? await listCompanyProfessionals(auth.auth.companyId, search, status, {}, unitId) : await listCompanyProfessionals(auth.auth.companyId, search, status); return NextResponse.json({ ok: true, items }); }
