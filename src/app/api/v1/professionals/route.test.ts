...
describe("professionals API tenant boundary", () => { beforeEach(() => vi.clearAllMocks());
  ...
  it("ignores external company identifiers", async () => { mocks.auth.mockResolvedValue({ ok: true, auth: { companyId: "company-a", role: "staff" } }); mocks.list.mockResolvedValue([]); await GET(new Request("https://sisag.test/api/v1/professionals?companyId=company-b") as never); expect(mocks.list).toHaveBeenCalledWith("company-a", "", "active"); });
  it("accepts an explicit inactive filter", async () => { mocks.auth.mockResolvedValue({ ok: true, auth: { companyId: "company-a", role: "admin" } }); mocks.list.mockResolvedValue([]); await GET(new Request("https://sisag.test/api/v1/professionals?status=inactive") as never); expect(mocks.list).toHaveBeenCalledWith("company-a", "", "inactive"); });
