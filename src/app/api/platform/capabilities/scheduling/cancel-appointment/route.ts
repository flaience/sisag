...
type CancelAppointmentRequestBody = {
  companyId?: string;
  actorId?: string;
...
export async function POST(request: Request) {
  ...
  try {
    const body = (await request.json()) as CancelAppointmentRequestBody;
    ...
    if (!companyId) {
      return jsonError("SCHEDULING_COMPANY_REQUIRED", "companyId é obrigatório.", 400);
    }
    if (!uuidRe.test(companyId)) {
      return jsonError("SCHEDULING_INVALID_COMPANY_ID", "companyId inválido.", 400);
    }
    if (!appointmentId) {
      return jsonError(
    ...
    console.error("PLATFORM CANCEL APPOINTMENT ERROR:", error);
    return NextResponse.json(
      {
        ok: false,
        error: {
