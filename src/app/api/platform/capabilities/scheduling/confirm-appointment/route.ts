// src/app/api/platform/capabilities/scheduling/confirm-appointment/route.ts
...
type ConfirmAppointmentRequestBody = {
  companyId?: string;
  actorId?: string;
...
export async function POST(request: Request) {
  ...
  try {
    const body = (await request.json()) as ConfirmAppointmentRequestBody;
    ...
    const result = await adapter.confirmAppointment(context, { appointmentId });
    ...
    console.error("PLATFORM CONFIRM APPOINTMENT ERROR:", error);
        ...
        error: {
          ...
          message:
            error instanceof Error
              ? error.message
              : "Erro inesperado ao confirmar agendamento.",
        },
      },
      { status: 500 },
    );
