...
export const recoveryAgentRetrievalReleaseCandidates = pgTable("recovery_agent_retrieval_release_candidates", { id: uuid("id").defaultRandom().primaryKey(), companyId: uuid("company_id").notNull().references(() => companies.id, { onDelete: "cascade" }), evaluationId: uuid("evaluation_id").notNull().references(() => recoveryAgentRetrievalExperimentEvaluations.id), experimentId: uuid("experiment_id").notNull().references(() => recoveryAgentRetrievalShadowExperiments.id), proposalId: uuid("proposal_id").notNull().references(() => recoveryAgentRetrievalChangeProposals.id), scope: varchar("scope", { length: 32 }).notNull(), status: varchar("status", { length: 16 }).notNull().default("draft"), candidateVersion: varchar("candidate_version", { length: 64 }).notNull(), candidate: jsonb("candidate").notNull(), evidence: jsonb("evidence").notNull(), reason: text("reason").notNull(), createdBy: uuid("created_by").notNull(), createdAt: timestamp("created_at", { withTimezone: true }).notNull().defaultNow(), reviewedBy: uuid("reviewed_by"), reviewedAt: timestamp("reviewed_at", { withTimezone: true }), reviewReason: text("review_reason") }, t => ({ evaluationUq:uniqueIndex("recovery_retrieval_release_candidates_evaluation_uq").on(t.evaluationId), companyIdx:index("recovery_retrieval_release_candidates_company_idx").on(t.companyId,t.createdAt), statusCheck:check("recovery_retrieval_release_candidates_status_check",sql`${t.status} in ('draft','approved','rejected')`), reasonCheck:check("recovery_retrieval_release_candidates_reason_check",sql`char_length(${t.reason}) between 3 and 500`), reviewReasonCheck:check("recovery_retrieval_release_candidates_review_reason_check",sql`${t.reviewReason} is null or char_length(${t.reviewReason}) between 3 and 500`) })).enableRLS();
  ...
  export const recoveryAgentRetrievalReleaseGraduationProposals = pgTable("recovery_agent_retrieval_release_graduation_proposals", { id:uuid("id").defaultRandom().primaryKey(),companyId:uuid("company_id").notNull().references(()=>companies.id,{onDelete:"cascade"}),releasePlanId:uuid("release_plan_id").notNull().references(()=>recoveryAgentRetrievalReleasePlans.id),releaseCandidateId:uuid("release_candidate_id").notNull().references(()=>recoveryAgentRetrievalReleaseCandidates.id),scope:varchar("scope",{length:32}).notNull(),status:varchar("status",{length:16}).notNull().default("proposed"),rolloutPercent:integer("rollout_percent").notNull(),healthPolicyVersion:varchar("health_policy_version",{length:100}).notNull(),graduationPolicyVersion:varchar("graduation_policy_version",{length:100}).notNull(),evidence:jsonb("evidence").notNull(),reason:text("reason").notNull(),createdBy:uuid("created_by").notNull(),createdAt:timestamp("created_at",{withTimezone:true}).notNull().defaultNow(),version:integer("version").notNull().default(1),reviewedBy:uuid("reviewed_by"),reviewedAt:timestamp("reviewed_at",{withTimezone:true}),reviewReason:text("review_reason"),reviewedVersion:integer("reviewed_version") },t=>({proposedUq:uniqueIndex("recovery_retrieval_release_graduation_proposed_uq").on(t.companyId,t.releasePlanId).where(sql`${t.status} = 'proposed'`),companyIdx:index("recovery_retrieval_release_graduation_company_idx").on(t.companyId,t.createdAt),statusCheck:check("recovery_retrieval_release_graduation_status_check",sql`${t.status} in ('proposed','approved','rejected')`),rolloutCheck:check("recovery_retrieval_release_graduation_rollout_check",sql`${t.rolloutPercent} = 100`),reasonCheck:check("recovery_retrieval_release_graduation_reason_check",sql`char_length(${t.reason}) between 3 and 500`) })).enableRLS();
