...
export const recoveryAgentRetrievalChangeProposals = pgTable("recovery_agent_retrieval_change_proposals", { id: uuid("id").defaultRandom().primaryKey(), companyId: uuid("company_id").notNull().references(() => companies.id, { onDelete: "cascade" }), baselineId: uuid("baseline_id").notNull().references(() => recoveryAgentRetrievalBaselines.id), scope: varchar("scope", { length: 32 }).notNull().default("recovery"), status: varchar("status", { length: 16 }).notNull().default("proposed"), baselineReportHash: varchar("baseline_report_hash", { length: 64 }).notNull(), currentReportHash: varchar("current_report_hash", { length: 64 }).notNull(), candidateVersion: varchar("candidate_version", { length: 64 }).notNull(), candidate: jsonb("candidate").notNull(), comparison: jsonb("comparison").notNull(), reason: text("reason").notNull(), createdBy: uuid("created_by").notNull(), createdAt: timestamp("created_at", { withTimezone: true }).notNull().defaultNow() }, t => ({ companyCreatedIdx:index("recovery_retrieval_change_proposals_company_created_idx").on(t.companyId,t.createdAt), statusCheck:check("recovery_retrieval_change_proposals_status_check",sql`${t.status} = 'proposed'`), hashCheck:check("recovery_retrieval_change_proposals_hash_check",sql`char_length(${t.baselineReportHash}) = 64 and char_length(${t.currentReportHash}) = 64`), reasonCheck:check("recovery_retrieval_change_proposals_reason_check",sql`char_length(${t.reason}) between 3 and 500`) })).enableRLS();
