...
export const bookingRecoveryRecommendations = pgTable(
  "booking_recovery_recommendations",
  {
    ...
    decidedPriority: varchar("decided_priority", { length: 16 }),
...
export const recoveryAgentRetrievalBaselines = pgTable("recovery_agent_retrieval_baselines", { id: uuid("id").defaultRandom().primaryKey(), companyId: uuid("company_id").notNull().references(() => companies.id, { onDelete: "cascade" }), scope: varchar("scope", { length: 32 }).notNull().default("recovery"), status: varchar("status", { length: 16 }).notNull().default("active"), reportHash: varchar("report_hash", { length: 64 }).notNull(), datasetHash: varchar("dataset_hash", { length: 64 }).notNull(), reportSchemaVersion: varchar("report_schema_version", { length: 64 }).notNull(), qualityPolicyVersion: varchar("quality_policy_version", { length: 64 }).notNull(), samplingPolicyVersion: varchar("sampling_policy_version", { length: 64 }).notNull(), reason: text("reason").notNull(), approvedBy: uuid("approved_by").notNull(), approvedAt: timestamp("approved_at", { withTimezone: true }).notNull().defaultNow(), retiredBy: uuid("retired_by"), retiredAt: timestamp("retired_at", { withTimezone: true }), retirementReason: text("retirement_reason") }, t => ({ companyScopeIdx:index("recovery_retrieval_baselines_company_scope_idx").on(t.companyId,t.scope,t.approvedAt), activeUq:uniqueIndex("recovery_retrieval_baselines_active_uq").on(t.companyId,t.scope).where(sql`${t.status} = 'active'`), statusCheck:check("recovery_retrieval_baselines_status_check",sql`${t.status} in ('active','retired')`), hashCheck:check("recovery_retrieval_baselines_hash_check",sql`char_length(${t.reportHash}) = 64 and char_length(${t.datasetHash}) = 64`), reasonCheck:check("recovery_retrieval_baselines_reason_check",sql`char_length(${t.reason}) between 3 and 500`) })).enableRLS();
export const recoveryAgentRetrievalChangeProposals = pgTable("recovery_agent_retrieval_change_proposals", { id: uuid("id").defaultRandom().primaryKey(), companyId: uuid("company_id").notNull().references(() => companies.id, { onDelete: "cascade" }), baselineId: uuid("baseline_id").notNull().references(() => recoveryAgentRetrievalBaselines.id), scope: varchar("scope", { length: 32 }).notNull().default("recovery"), status: varchar("status", { length: 16 }).notNull().default("proposed"), baselineReportHash: varchar("baseline_report_hash", { length: 64 }).notNull(), currentReportHash: varchar("current_report_hash", { length: 64 }).notNull(), candidateVersion: varchar("candidate_version", { length: 64 }).notNull(), candidate: jsonb("candidate").notNull(), comparison: jsonb("comparison").notNull(), reason: text("reason").notNull(), createdBy: uuid("created_by").notNull(), createdAt: timestamp("created_at", { withTimezone: true }).notNull().defaultNow(), reviewedBy: uuid("reviewed_by"), reviewedAt: timestamp("reviewed_at", { withTimezone: true }), reviewReason: text("review_reason") }, t => ({ companyCreatedIdx:index("recovery_retrieval_change_proposals_company_created_idx").on(t.companyId,t.createdAt), statusCheck:check("recovery_retrieval_change_proposals_status_check",sql`${t.status} in ('proposed','approved','rejected')`), hashCheck:check("recovery_retrieval_change_proposals_hash_check",sql`char_length(${t.baselineReportHash}) = 64 and char_length(${t.currentReportHash}) = 64`), reasonCheck:check("recovery_retrieval_change_proposals_reason_check",sql`char_length(${t.reason}) between 3 and 500`), reviewReasonCheck:check("recovery_retrieval_change_proposals_review_reason_check",sql`${t.reviewReason} is null or char_length(${t.reviewReason}) between 3 and 500`) })).enableRLS();
export const recoveryAgentRetrievalShadowExperiments = pgTable("recovery_agent_retrieval_shadow_experiments", { id: uuid("id").defaultRandom().primaryKey(), companyId: uuid("company_id").notNull().references(() => companies.id, { onDelete: "cascade" }), proposalId: uuid("proposal_id").notNull().references(() => recoveryAgentRetrievalChangeProposals.id), scope: varchar("scope", { length: 32 }).notNull().default("recovery"), status: varchar("status", { length: 16 }).notNull().default("active"), sampleRate: integer("sample_rate").notNull(), maxTokensPerRun: integer("max_tokens_per_run").notNull(), maxDurationMs: integer("max_duration_ms").notNull(), startsAt: timestamp("starts_at", { withTimezone: true }).notNull(), endsAt: timestamp("ends_at", { withTimezone: true }).notNull(), reason: text("reason").notNull(), createdBy: uuid("created_by").notNull(), createdAt: timestamp("created_at", { withTimezone: true }).notNull().defaultNow(), stoppedBy: uuid("stopped_by"), stoppedAt: timestamp("stopped_at", { withTimezone: true }), stopReason: text("stop_reason") }, t => ({ companyScopeIdx:index("recovery_retrieval_shadow_experiments_company_scope_idx").on(t.companyId,t.scope,t.createdAt), activeUq:uniqueIndex("recovery_retrieval_shadow_experiments_active_uq").on(t.companyId,t.scope).where(sql`${t.status} = 'active'`), statusCheck:check("recovery_retrieval_shadow_experiments_status_check",sql`${t.status} in ('active','stopped')`), sampleCheck:check("recovery_retrieval_shadow_experiments_sample_check",sql`${t.sampleRate} between 1 and 100`), budgetCheck:check("recovery_retrieval_shadow_experiments_budget_check",sql`${t.maxTokensPerRun} between 1 and 10000 and ${t.maxDurationMs} between 100 and 20000`), windowCheck:check("recovery_retrieval_shadow_experiments_window_check",sql`${t.endsAt} > ${t.startsAt}`) })).enableRLS();

export const recoveryAgentRetrievalExperimentEvaluations = pgTable("recovery_agent_retrieval_experiment_evaluations", { id: uuid("id").defaultRandom().primaryKey(), companyId: uuid("company_id").notNull().references(() => companies.id, { onDelete: "cascade" }), experimentId: uuid("experiment_id").notNull().references(() => recoveryAgentRetrievalShadowExperiments.id), proposalId: uuid("proposal_id").notNull().references(() => recoveryAgentRetrievalChangeProposals.id), decision: varchar("decision", { length: 16 }).notNull(), reason: text("reason").notNull(), evidence: jsonb("evidence").notNull(), evaluatedBy: uuid("evaluated_by").notNull(), evaluatedAt: timestamp("evaluated_at", { withTimezone: true }).notNull().defaultNow() }, t => ({ experimentUq:uniqueIndex("recovery_retrieval_experiment_evaluations_experiment_uq").on(t.experimentId), companyIdx:index("recovery_retrieval_experiment_evaluations_company_idx").on(t.companyId,t.evaluatedAt), decisionCheck:check("recovery_retrieval_experiment_evaluations_decision_check",sql`${t.decision} in ('adopt','reject','inconclusive')`), reasonCheck:check("recovery_retrieval_experiment_evaluations_reason_check",sql`char_length(${t.reason}) between 3 and 500`) })).enableRLS();

export const recoveryAgentRetrievalReleaseCandidates = pgTable("recovery_agent_retrieval_release_candidates", { id: uuid("id").defaultRandom().primaryKey(), companyId: uuid("company_id").notNull().references(() => companies.id, { onDelete: "cascade" }), evaluationId: uuid("evaluation_id").notNull().references(() => recoveryAgentRetrievalExperimentEvaluations.id), experimentId: uuid("experiment_id").notNull().references(() => recoveryAgentRetrievalShadowExperiments.id), proposalId: uuid("proposal_id").notNull().references(() => recoveryAgentRetrievalChangeProposals.id), scope: varchar("scope", { length: 32 }).notNull(), status: varchar("status", { length: 16 }).notNull().default("draft"), candidateVersion: varchar("candidate_version", { length: 64 }).notNull(), candidate: jsonb("candidate").notNull(), evidence: jsonb("evidence").notNull(), reason: text("reason").notNull(), createdBy: uuid("created_by").notNull(), createdAt: timestamp("created_at", { withTimezone: true }).notNull().defaultNow(), reviewedBy: uuid("reviewed_by"), reviewedAt: timestamp("reviewed_at", { withTimezone: true }), reviewReason: text("review_reason") }, t => ({ evaluationUq:uniqueIndex("recovery_retrieval_release_candidates_evaluation_uq").on(t.evaluationId), companyIdx:index("recovery_retrieval_release_candidates_company_idx").on(t.companyId,t.createdAt), statusCheck:check("recovery_retrieval_release_candidates_status_check",sql`${t.status} in ('draft','approved','rejected')`), reasonCheck:check("recovery_retrieval_release_candidates_reason_check",sql`char_length(${t.reason}) between 3 and 500`), reviewReasonCheck:check("recovery_retrieval_release_candidates_review_reason_check",sql`${t.reviewReason} is null or char_length(${t.reviewReason}) between 3 and 500`) })).enableRLS();

  export const recoveryAgentRetrievalReleasePlans = pgTable("recovery_agent_retrieval_release_plans", { id: uuid("id").defaultRandom().primaryKey(), companyId: uuid("company_id").notNull().references(() => companies.id, { onDelete: "cascade" }), releaseCandidateId: uuid("release_candidate_id").notNull().references(() => recoveryAgentRetrievalReleaseCandidates.id), scope: varchar("scope", { length: 32 }).notNull(), status: varchar("status", { length: 16 }).notNull().default("scheduled"), rolloutPercent: integer("rollout_percent").notNull(), maxTokensPerRun: integer("max_tokens_per_run").notNull(), maxDurationMs: integer("max_duration_ms").notNull(), startsAt: timestamp("starts_at", { withTimezone: true }).notNull(), endsAt: timestamp("ends_at", { withTimezone: true }).notNull(), reason: text("reason").notNull(), createdBy: uuid("created_by").notNull(), createdAt: timestamp("created_at", { withTimezone: true }).notNull().defaultNow(), stoppedBy: uuid("stopped_by"), stoppedAt: timestamp("stopped_at", { withTimezone: true }), stopReason: text("stop_reason"), graduatedBy:uuid("graduated_by"),graduatedAt:timestamp("graduated_at",{withTimezone:true}),graduationReason:text("graduation_reason") }, t => ({ liveUq:uniqueIndex("recovery_retrieval_release_plans_live_uq").on(t.companyId,t.scope).where(sql`${t.status} in ('scheduled','graduated')`), companyIdx:index("recovery_retrieval_release_plans_company_idx").on(t.companyId,t.createdAt), statusCheck:check("recovery_retrieval_release_plans_status_check",sql`${t.status} in ('scheduled','stopped','graduated')`), rolloutCheck:check("recovery_retrieval_release_plans_rollout_check",sql`${t.rolloutPercent} between 1 and 100`), budgetCheck:check("recovery_retrieval_release_plans_budget_check",sql`${t.maxTokensPerRun} between 1 and 10000 and ${t.maxDurationMs} between 100 and 20000`), windowCheck:check("recovery_retrieval_release_plans_window_check",sql`${t.endsAt} > ${t.startsAt}`) })).enableRLS();
  export const recoveryAgentRetrievalReleaseProgressionProposals = pgTable("recovery_agent_retrieval_release_progression_proposals", { id:uuid("id").defaultRandom().primaryKey(),companyId:uuid("company_id").notNull().references(()=>companies.id,{onDelete:"cascade"}),releasePlanId:uuid("release_plan_id").notNull().references(()=>recoveryAgentRetrievalReleasePlans.id),scope:varchar("scope",{length:32}).notNull(),status:varchar("status",{length:16}).notNull().default("proposed"),currentRolloutPercent:integer("current_rollout_percent").notNull(),proposedRolloutPercent:integer("proposed_rollout_percent").notNull(),healthPolicyVersion:varchar("health_policy_version",{length:100}).notNull(),progressionPolicyVersion:varchar("progression_policy_version",{length:100}).notNull(),evidence:jsonb("evidence").notNull(),reason:text("reason").notNull(),createdBy:uuid("created_by").notNull(),createdAt:timestamp("created_at",{withTimezone:true}).notNull().defaultNow(),reviewedBy:uuid("reviewed_by"),reviewedAt:timestamp("reviewed_at",{withTimezone:true}),reviewReason:text("review_reason"),appliedBy:uuid("applied_by"),appliedAt:timestamp("applied_at",{withTimezone:true}),applicationReason:text("application_reason"),applicationEvidence:jsonb("application_evidence") },t=>({proposedUq:uniqueIndex("recovery_retrieval_release_progression_proposed_uq").on(t.companyId,t.releasePlanId).where(sql`${t.status} = 'proposed'`),companyIdx:index("recovery_retrieval_release_progression_company_idx").on(t.companyId,t.createdAt),statusCheck:check("recovery_retrieval_release_progression_status_check",sql`${t.status} in ('proposed','approved','rejected','applied')`),rolloutCheck:check("recovery_retrieval_release_progression_rollout_check",sql`${t.currentRolloutPercent} between 1 and 100 and ${t.proposedRolloutPercent} between 2 and 100 and ${t.proposedRolloutPercent} > ${t.currentRolloutPercent} and ${t.proposedRolloutPercent} <= least(${t.currentRolloutPercent} + 25, 100)`) })).enableRLS();
  export const recoveryAgentRetrievalReleaseRollbackProposals = pgTable("recovery_agent_retrieval_release_rollback_proposals", { id:uuid("id").defaultRandom().primaryKey(),companyId:uuid("company_id").notNull().references(()=>companies.id,{onDelete:"cascade"}),releasePlanId:uuid("release_plan_id").notNull().references(()=>recoveryAgentRetrievalReleasePlans.id),scope:varchar("scope",{length:32}).notNull(),status:varchar("status",{length:16}).notNull().default("proposed"),currentRolloutPercent:integer("current_rollout_percent").notNull(),proposedRolloutPercent:integer("proposed_rollout_percent").notNull(),healthPolicyVersion:varchar("health_policy_version",{length:100}).notNull(),rollbackPolicyVersion:varchar("rollback_policy_version",{length:100}).notNull(),evidence:jsonb("evidence").notNull(),reason:text("reason").notNull(),createdBy:uuid("created_by").notNull(),createdAt:timestamp("created_at",{withTimezone:true}).notNull().defaultNow(),reviewedBy:uuid("reviewed_by"),reviewedAt:timestamp("reviewed_at",{withTimezone:true}),reviewReason:text("review_reason"),appliedBy:uuid("applied_by"),appliedAt:timestamp("applied_at",{withTimezone:true}),applicationReason:text("application_reason"),applicationEvidence:jsonb("application_evidence") },t=>({proposedUq:uniqueIndex("recovery_retrieval_release_rollback_proposed_uq").on(t.companyId,t.releasePlanId).where(sql`${t.status} = 'proposed'`),companyIdx:index("recovery_retrieval_release_rollback_company_idx").on(t.companyId,t.createdAt),statusCheck:check("recovery_retrieval_release_rollback_status_check",sql`${t.status} in ('proposed','approved','rejected','applied')`),rolloutCheck:check("recovery_retrieval_release_rollback_rollout_check",sql`${t.currentRolloutPercent} between 2 and 100 and ${t.proposedRolloutPercent} between 1 and 99 and ${t.proposedRolloutPercent} < ${t.currentRolloutPercent} and ${t.proposedRolloutPercent} >= greatest(${t.currentRolloutPercent} - 25, 1)`) })).enableRLS();
  export const recoveryAgentRetrievalReleaseGraduationProposals = pgTable("recovery_agent_retrieval_release_graduation_proposals", { id:uuid("id").defaultRandom().primaryKey(),companyId:uuid("company_id").notNull().references(()=>companies.id,{onDelete:"cascade"}),releasePlanId:uuid("release_plan_id").notNull().references(()=>recoveryAgentRetrievalReleasePlans.id),releaseCandidateId:uuid("release_candidate_id").notNull().references(()=>recoveryAgentRetrievalReleaseCandidates.id),scope:varchar("scope",{length:32}).notNull(),status:varchar("status",{length:16}).notNull().default("proposed"),rolloutPercent:integer("rollout_percent").notNull(),healthPolicyVersion:varchar("health_policy_version",{length:100}).notNull(),graduationPolicyVersion:varchar("graduation_policy_version",{length:100}).notNull(),evidence:jsonb("evidence").notNull(),reason:text("reason").notNull(),createdBy:uuid("created_by").notNull(),createdAt:timestamp("created_at",{withTimezone:true}).notNull().defaultNow(),version:integer("version").notNull().default(1),reviewedBy:uuid("reviewed_by"),reviewedAt:timestamp("reviewed_at",{withTimezone:true}),reviewReason:text("review_reason"),reviewedVersion:integer("reviewed_version"),appliedBy:uuid("applied_by"),appliedAt:timestamp("applied_at",{withTimezone:true}),applicationReason:text("application_reason"),applicationEvidence:jsonb("application_evidence") },t=>({proposedUq:uniqueIndex("recovery_retrieval_release_graduation_proposed_uq").on(t.companyId,t.releasePlanId).where(sql`${t.status} = 'proposed'`),companyIdx:index("recovery_retrieval_release_graduation_company_idx").on(t.companyId,t.createdAt),statusCheck:check("recovery_retrieval_release_graduation_status_check",sql`${t.status} in ('proposed','approved','rejected','applied')`),rolloutCheck:check("recovery_retrieval_release_graduation_rollout_check",sql`${t.rolloutPercent} = 100`),reasonCheck:check("recovery_retrieval_release_graduation_reason_check",sql`char_length(${t.reason}) between 3 and 500`) })).enableRLS();
...
export const whatsappAudioProcessing = pgTable("whatsapp_audio_processing", {
  ...
  dispatchErrorCode: varchar("dispatch_error_code", { length: 64 }),
  ...
  dispatchAttemptsCheck: check("whatsapp_audio_processing_dispatch_attempts_check", sql`${t.dispatchAttempts} between 0 and 3`),
