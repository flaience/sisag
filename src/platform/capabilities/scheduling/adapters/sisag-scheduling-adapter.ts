    ...
    try {
      ...
      if (!companyId || !appointmentId) {
        return {
          ...
          error: {
            code: "SCHEDULING_OPERATION_NOT_ALLOWED",
            message: "Empresa e agendamento são obrigatórios para conclusão.",
          },
        };
      }
      ...
      const completed = await BookingCoreService.completeById({
        companyId,
        clientId: booking.clientId,
        bookingId: appointmentId,
        actor: actorMap[context.actor.type] ?? "system",
      ...
      if (completed.ok === false) {
        return {
          ...
          error: {
            code: "SCHEDULING_OPERATION_NOT_ALLOWED",
            message:
              "O agendamento não existe mais ou não pode ser concluído no estado atual.",
          },
        };
      }
      ...
      console.error(
        "SISAG SCHEDULING ADAPTER COMPLETE APPOINTMENT ERROR:",
        error,
      );
