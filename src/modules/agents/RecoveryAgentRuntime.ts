import { RecoveryAgentDecisionSchema, type RecoveryAgentDecision } from "./RecoveryAgentDecision.schema";
...
export type AgentProviderRequest = { system: string; input: unknown; schemaName: string; timeoutMs: number };
...
export async function executeRecoveryAgent(input: { context: RecoveryAgentContext; provider?: RecoveryAgentProvider; providerName?: string; timeoutMs?: number }) {
  ...
  const timeoutMs = Math.min(Math.max(input.timeoutMs ?? 8000, 1000), 20000);
  let timer: ReturnType<typeof setTimeout> | undefined;
  try {
    const timeout = new Promise<never>((_, reject) => { timer = setTimeout(() => reject(new Error("provider_timeout")), timeoutMs); });
    const response = await Promise.race([input.provider.complete({ system: buildRecoveryAgentPrompt(), input: input.context, schemaName: "recovery_agent_decision", timeoutMs }), timeout]);
