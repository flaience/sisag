...
describe("agent outcomes", () => {
  ...
  it("publishes experiment observability",()=>{const result=summarizeAgentObservability([row({agentExecution:{mode:"ai",retrievalShadow:{mode:"ai",totalTokens:8,durationMs:20,experiment:{id:"e",selected:true}}}}),row({agentExecution:{mode:"ai",retrievalShadow:{mode:"fallback",errorCode:"outside_experiment_sample",experiment:{id:"e",selected:false}}}})]);expect(result.retrieval.experiments).toMatchObject({observations:2,selected:1,excluded:1,experiments:[{experimentId:"e",selectionRate:50,aiRuns:1}]})});
  it("publishes release canary observability",()=>{const result=summarizeAgentObservability([row({agentExecution:{mode:"ai",retrievalShadow:{mode:"ai",totalTokens:8,durationMs:20,release:{id:"p",selected:true}}}}),row({agentExecution:{mode:"ai",retrievalShadow:{mode:"fallback",errorCode:"outside_release_rollout",release:{id:"p",selected:false}}}})]);expect(result.retrieval.releaseCanaries).toMatchObject({observations:2,selected:1,excluded:1,plans:[{planId:"p",selectionRate:50,aiRuns:1}]})});
  it("publishes conservative health for each release canary",()=>{const runs=Array.from({length:20},()=>row({agentExecution:{mode:"ai",retrievalShadow:{mode:"ai",totalTokens:8,durationMs:20,overlapRate:70,release:{id:"p",selected:true}}}}));expect(summarizeAgentObservability(runs).retrieval.releaseCanaries.health).toMatchObject({automaticAction:false,plans:[{planId:"p",status:"healthy"}]})});
  it("publishes progression eligibility without automatic expansion",()=>{const runs=Array.from({length:20},()=>row({agentExecution:{mode:"ai",retrievalShadow:{mode:"ai",totalTokens:8,durationMs:20,overlapRate:70,release:{id:"p",selected:true}}}}));expect(summarizeAgentObservability(runs).retrieval.releaseCanaries.progression).toMatchObject({requiresHumanApproval:true,automaticExpansion:false,plans:[{planId:"p",status:"eligible_for_expansion"}]})});
});
