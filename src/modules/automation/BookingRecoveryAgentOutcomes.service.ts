...
import { evaluateRecoveryRetrievalSample } from "@/modules/agents/RecoveryRetrievalSamplingGuardrails";
...
export function summarizeAgentObservability(rows: OutcomeRow[], evaluationRows: RetrievalEvaluationRow[] = []) {
  ...
  const sampling = evaluateRecoveryRetrievalSample(evaluationRows);
  ...
  return {
    ...
    retrieval:{executions:retrievals.length,vectorRuns,fallbackRuns:retrievalFallbackRuns,availabilityRate:pct(vectorRuns,retrievals.length),fallbackRate:pct(retrievalFallbackRuns,retrievals.length),averageOverlapRate,totalTokens:retrievalTokens,averageTokens:average(retrievals.map(x=>numberOrZero(x.totalTokens))),averageDurationMs:average(retrievalDurations),p95DurationMs:Math.round(percentile95(retrievalDurations)),errors:[...new Set(retrievals.map(x=>text(x.errorCode)).filter((x):x is string=>Boolean(x)))].map(errorCode=>({errorCode,count:retrievals.filter(x=>x.errorCode===errorCode).length})),humanEvaluation:humanRetrieval,sampling,qualityGate:retrievalQuality,experiments:experimentMetrics},
    providers,
    errors,
    engines,
  };
