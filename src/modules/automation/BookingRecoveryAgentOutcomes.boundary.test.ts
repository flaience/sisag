...
describe("agent outcomes boundary", () => {
  ...
  it("measures shadow execution without changing authority", () => {
    for (const field of ["agentDecision", "agentExecution", "fallbackRate", "agentHumanAgreementRate", "totalTokens", "p95DurationMs", "retrievalShadow", "averageOverlapRate", "evaluateRecoveryRetrievalQuality", "summarizeRecoveryRetrievalExperiments"]) expect(service).toContain(field);
